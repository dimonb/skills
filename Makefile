...
# (scripts/check.sh) and then the driver, flow and adapter suites, which are fast enough to gate
# every commit — so a regression in any of the three reds a commit. The shipyard and council
# suites are too slow for a per-commit gate; `make test` runs those. What `make check` DOES gate
# for all five suites, statically, is registration: a test file that stops being listed in its
# run-all.sh reds here (scripts/check.sh check 10), so a suite cannot silently stop running.
check:
