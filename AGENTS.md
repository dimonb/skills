...
* **A throwaway script that builds a plugin's runtime state owns its teardown.** A council
  ...
  does, so a probe script that builds one under a fixed path and never removes it leaves that
  process behind, and what is left behind accumulates until the machine reboots. Build under
  a fresh `mktemp -d` root with `trap 'rm -rf "$ROOT"' EXIT`, or source the plugin's own test
  helpers, which already do both. `make check` cannot see this; a process count after the
  script exits can, so take one.
