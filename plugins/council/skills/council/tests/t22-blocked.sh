#!/usr/bin/env bash
# t22 — a blocked participant is told apart from a thinking one, and the supervisor is TOLD.
#
...
ok "a self-healing wait wakes nobody"        0 "$(notices t22a)"
...
ok "an unexplained stall pushes a notice"    1 "$(notices t22g)"
