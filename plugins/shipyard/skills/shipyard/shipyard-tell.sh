...
#       your eyes, 3 the backend answered and has no terminal for that slot (the child IS gone),
#       7 UNRESOLVED — no terminal on the backend this run resolved, and that backend either did
#       not answer or is not the one this fleet was launched on, so whether the child is alive is
#       UNKNOWN, 2 usage error, 1 mailbox/backend failure.
