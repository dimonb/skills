#!/usr/bin/env bash
# sh-answer.sh — the PARENT watcher's reply to a child ship session's escalation.
#
# Usage:
#   sh-answer.sh <id> "<the human's answer / decision>"
#   sh-answer.sh <id> @<file>           payload read VERBATIM from a file
#   sh-answer.sh <id> @-                payload read VERBATIM from stdin
#
...
#   sh-answer.sh --list                 list every escalation (id / slot / status)
#   sh-answer.sh --no-tell <id> "..."   write the record only, never fall back
#
# For a `question` or a `decision` the child picks the answer up on its own
# (sh-ask.sh blocks or polls) and keeps going — you never type into its window.
#
...
# script does NOT pretend to have answered — it hands the text to sh-tell.sh,
...
MB=$(sh_mailbox) || { echo "error: not inside a git repository" >&2; exit 1; }
...
if [ -z "$ID" ] || [ -z "$ANS_RAW" ]; then
  echo 'usage: sh-answer.sh [--no-tell] <id> "<answer>"        (short, single-line)' >&2
  echo '       sh-answer.sh [--no-tell] <id> @<file>           (verbatim, RECOMMENDED)' >&2
  echo '       sh-answer.sh [--no-tell] <id> @-              (verbatim, from stdin)' >&2
  echo '       sh-answer.sh --list' >&2
  echo >&2
...
if [ "$KIND" = "notice" ] || [ "$ST" = "done" ]; then
  if [ "$NO_TELL" = 1 ]; then
    ...
    if bash "$DIR/sh-tell.sh" "$ID" "$ANS"; then exit 0; fi
